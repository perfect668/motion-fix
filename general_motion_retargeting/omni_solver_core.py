...
class OmniSolverCore:
    """Shared Omni-first kinematic solver used by the standalone V4 pipeline.

    This core deliberately contains no historical entry-point dependency.
    It owns the interaction Laplacian, terrain guards, self-collision limits,
    posture regularisation, and Mink QP iteration used by :class:`WholeBodyV4`.
    """
    ...
    def __init__(
        ...
        if not robot_xml.is_absolute():
            # Generated per-run configs normally live under ``outputs/``;
            # their relative robot path is still project-relative.  First
            # honour a config-local asset, then resolve against this clean
            # package's root.  This never reaches into the source archive.
            candidates = (
                self.config_path.parent / robot_xml,
                Path(__file__).resolve().parents[1] / robot_xml,
            )
            robot_xml = next((path for path in candidates if path.is_file()), candidates[-1])
        ...
        for joint_name, bounds in self.config.get("joint_position_limits", {}).items():
            ...
            if self.model.jnt_type[joint_id] not in (
                ...
                raise ValueError(f"V4 custom limit requires a hinge or slide joint: {joint_name}")
            ...
            if not lower < upper:
                raise ValueError(f"Invalid V4 joint range for {joint_name}: {bounds}")
    ...
    def _source_semantics(self, source_frame: dict[str, np.ndarray]) -> dict[str, np.ndarray]:
        ...
        if missing:
            raise ValueError(f"Source frame is missing V4 semantic joints: {sorted(set(missing))}")
        return {
