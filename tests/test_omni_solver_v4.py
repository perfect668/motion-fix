...
from general_motion_retargeting.omni_solver_core import (
...
CONFIG = ROOT / "general_motion_retargeting/ik_configs/v4_defaults.json"


def test_v4_config_uses_native_holosoma_semantics_without_smpl_tables():
...
def test_v4_applies_ne01_torso_roll_safety_range():
    retargeter = OmniSolverCore(
        CONFIG,
        TerrainField(),
        np.array([
            [-0.5, -0.5, 0.0],
...
def test_v4_custom_joint_limit_cannot_expand_xml_range(tmp_path):
    config = json.loads(CONFIG.read_text())
    ...
    retargeter = OmniSolverCore(path, TerrainField(), np.zeros((4, 3)))
