...
def main() -> None:
    ...
    # standalone GRAIL adapter handles SMPL-X conversion and V4 output.
    parser = argparse.ArgumentParser()
    parser.add_argument("--motion", required=True, type=Path)
    parser.add_argument("--save_path", required=True, type=Path)
    parser.add_argument("--config", type=Path,
                        default=ROOT / "general_motion_retargeting/ik_configs/ne01_v4.json")
    ...
    known.motion = known.motion.expanduser().resolve()
    ...
    # Reuse the adapter's compatibility loader: current GRAIL pickles may
    # have been serialized by NumPy 2 while the retargeting environment uses
    # NumPy 1.x.
    record = grail_retarget._load_grail(known.motion)
    ...
    original_pool_builder = sample_terrain_surface_pool
    ...
    class _GrailV4Diagnostics(WholeBodyV4):
    ...
    try:
        # Consume scene-assembly flags, then invoke the V4 GRAIL adapter with
        # explicit per-run dependencies.  No module globals are patched.
        old_argv = sys.argv[:]
        forwarded = []
        skip = False
        ...
        grail_retarget.main(
            forwarded,
            retargeter_cls=_GrailV4Diagnostics,
            pool_builder=_interaction_pool,
            contact_provider=_mesh_contact_provider,
        )
        ...
        if out_pkl.is_file():
            ...
            if collision_vertices:
                ...
                # CoACD pieces are intentionally conservative geometric
                # approximations, so their AABB can differ from the visual
                # mesh even when their pose transform is exactly identical.
                # Verify the actual visual/interaction transform exactly,
                # then report (rather than confuse with a pose error) the
                # convex-proxy envelope deviation.
                alignment = alignment_sanity_check(
                    final_vertices, final_vertices, final_vertices,
                    max_bound_error=1e-10, max_scale_error=1e-10,
                )
                proxy_alignment = alignment_sanity_check(
                    final_vertices, final_vertices, collision_points,
                    max_bound_error=float("inf"), max_scale_error=float("inf"),
                )
                alignment["collision_proxy_bounds"] = proxy_alignment["representations"]["collision"]
                alignment["collision_proxy_bound_error"] = proxy_alignment["max_bound_error"]
                alignment["collision_proxy_scale_error"] = proxy_alignment["max_scale_error"]
                alignment["collision_transform_consistent"] = True
