...
def main() -> None:
    ...
    if args.terrain and config.get("scene_collision", {}).get("backend") in {"mujoco", "hybrid"}:
        ...
        effective_config = temporary_config
    try:
        retargeter = WholeBodyV4(effective_config, terrain, pool, fps=args.tgt_fps, solver=args.solver)
    ...
    payload = {
        ...
        "algorithm": "wholebody_omni_v4", "source_format": motion.source_format,
