...
BODY_TO_MUJOCO.update({name: name for name in BODY_NAMES if name not in BODY_TO_MUJOCO})
...
def convert_file(source: pathlib.Path, target: pathlib.Path, model: mujoco.MjModel) -> None:
    ...
    body_ids = [model.body(BODY_TO_MUJOCO[name]).id for name in BODY_NAMES]
    ...
    body_ang_vel = np.stack([angular_velocity(body_quat[:, i], dt) for i in range(len(BODY_NAMES))], axis=1)
