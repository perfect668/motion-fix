"""Recursive batch entry point for Laplacian soft-contact retargeting."""
...
def process_file(arguments):
    ...
    try:
        ...
        qpos = np.asarray(
            [retargeter.retarget(frame_data[index]).copy() for index in indices],
            dtype=float,
        )
        if len(qpos) == 0:
        ...
        try:
            ...
            with torch.no_grad():
                zero_root = torch.zeros(
                    (len(qpos), 3), dtype=torch.float32, device=resolved_device
                )
                identity_root = torch.zeros(
                    (len(qpos), 4), dtype=torch.float32, device=resolved_device
                )
                identity_root[:, -1] = 1.0
                local_body_pos_t, _ = kinematics.forward_kinematics(
                    ...
                    identity_root,
                    torch.as_tensor(
                        dof_pos, dtype=torch.float32, device=resolved_device
                    ),
                )
...
def main() -> None:
    ...
    for source_path in sorted(source_root.rglob("*.npz")):
        if source_path.name.endswith("_stagei.npz"):
            continue
        relative = source_path.relative_to(source_root).with_suffix(".pkl")
    ...
    if args.num_cpus <= 1:
        ...
        with context.Pool(args.num_cpus) as pool:
            for source_path, error in tqdm(
                pool.imap_unordered(process_file, jobs), total=len(jobs)
            ):
                if error:
                    failures.append((source_path, error))

    if failures:
        report = target_root / "laplacian_soft_failures.txt"
        report.parent.mkdir(parents=True, exist_ok=True)
        report.write_text(
            "\n\n".join(f"{path}\n{error}" for path, error in failures),
            encoding="utf-8",
        )
        raise RuntimeError(f"{len(failures)} motions failed; see {report}")
    print(f"Completed {len(jobs)} motions in {target_root}")
